# 75. Income Vs Expense Report Module — Full Module Specification

## Main Purpose
The **Income Vs Expense Report Module** is responsible for profit/cashflow style income-expense summaries in **Tradie Job Manager**.
...
## Core Permissions
```txt
reports.income_expense.view
```

## Suggested Go Services
```txt
IncomeExpenseReportService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/income_vs_expense_report_module
POST   /api/income_vs_expense_report_module
GET    /api/income_vs_expense_report_module/:id
PATCH  /api/income_vs_expense_report_module/:id
DELETE /api/income_vs_expense_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/income_vs_expense_report_module
```
...
```txt
INCOME_VS_EXPENSE_REPORT_MODULE_VIEWED
INCOME_VS_EXPENSE_REPORT_MODULE_CREATED
INCOME_VS_EXPENSE_REPORT_MODULE_UPDATED
INCOME_VS_EXPENSE_REPORT_MODULE_DELETED
INCOME_VS_EXPENSE_REPORT_MODULE_ACCESS_DENIED
INCOME_VS_EXPENSE_REPORT_MODULE_EXPORTED
```
