# 81. SWMS Document Module — Full Module Specification

## Main Purpose
The **SWMS Document Module** is responsible for Safe Work Method Statement storage and assignment in **Tradie Job Manager**.
...
```txt
GET    /api/swms_document_module
POST   /api/swms_document_module
GET    /api/swms_document_module/:id
PATCH  /api/swms_document_module/:id
DELETE /api/swms_document_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/swms_document_module
```
...
```txt
SWMS_DOCUMENT_MODULE_VIEWED
SWMS_DOCUMENT_MODULE_CREATED
SWMS_DOCUMENT_MODULE_UPDATED
SWMS_DOCUMENT_MODULE_DELETED
SWMS_DOCUMENT_MODULE_ACCESS_DENIED
SWMS_DOCUMENT_MODULE_EXPORTED
```
