# 35. Job Status Tracking Module — Full Module Specification

## Main Purpose
The **Job Status Tracking Module** is responsible for status transitions and job progress timeline in **Tradie Job Manager**.
...
```txt
GET    /api/job_status_tracking_module
POST   /api/job_status_tracking_module
GET    /api/job_status_tracking_module/:id
PATCH  /api/job_status_tracking_module/:id
DELETE /api/job_status_tracking_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/job_status_tracking_module
```
...
```txt
JOB_STATUS_TRACKING_MODULE_VIEWED
JOB_STATUS_TRACKING_MODULE_CREATED
JOB_STATUS_TRACKING_MODULE_UPDATED
JOB_STATUS_TRACKING_MODULE_DELETED
JOB_STATUS_TRACKING_MODULE_ACCESS_DENIED
JOB_STATUS_TRACKING_MODULE_EXPORTED
```
