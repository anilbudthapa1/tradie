# 60. Discount Module — Full Module Specification

## Main Purpose
The **Discount Module** is responsible for apply controlled quote/invoice discounts in **Tradie Job Manager**.
...
```txt
GET    /api/discount_module
POST   /api/discount_module
GET    /api/discount_module/:id
PATCH  /api/discount_module/:id
DELETE /api/discount_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/discount_module
```
...
```txt
DISCOUNT_MODULE_VIEWED
DISCOUNT_MODULE_CREATED
DISCOUNT_MODULE_UPDATED
DISCOUNT_MODULE_DELETED
DISCOUNT_MODULE_ACCESS_DENIED
DISCOUNT_MODULE_EXPORTED
```
