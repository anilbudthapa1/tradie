# 20. Lead Management Module — Full Module Specification

## Main Purpose
The **Lead Management Module** is responsible for lead capture, lead status, quote conversion and follow-up in **Tradie Job Manager**.
...
```txt
GET    /api/lead_management_module
POST   /api/lead_management_module
GET    /api/lead_management_module/:id
PATCH  /api/lead_management_module/:id
DELETE /api/lead_management_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/lead_management_module
```
...
```txt
LEAD_MANAGEMENT_MODULE_VIEWED
LEAD_MANAGEMENT_MODULE_CREATED
LEAD_MANAGEMENT_MODULE_UPDATED
LEAD_MANAGEMENT_MODULE_DELETED
LEAD_MANAGEMENT_MODULE_ACCESS_DENIED
LEAD_MANAGEMENT_MODULE_EXPORTED
```
