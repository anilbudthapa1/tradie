# 28. Payroll Module — Full Module Specification

## Main Purpose
The **Payroll Module** is responsible for pay runs, pay calculations and payroll reports in **Tradie Job Manager**.
...
```txt
GET    /api/payroll_module
POST   /api/payroll_module
GET    /api/payroll_module/:id
PATCH  /api/payroll_module/:id
DELETE /api/payroll_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/payroll_module
```
...
```txt
PAYROLL_MODULE_VIEWED
PAYROLL_MODULE_CREATED
PAYROLL_MODULE_UPDATED
PAYROLL_MODULE_DELETED
PAYROLL_MODULE_ACCESS_DENIED
PAYROLL_MODULE_EXPORTED
```
