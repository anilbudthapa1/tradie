# 76. GST BAS Report Module — Full Module Specification

## Main Purpose
The **GST BAS Report Module** is responsible for GST collected/paid summary for BAS support in **Tradie Job Manager**.
...
```txt
GET    /api/gst_bas_report_module
POST   /api/gst_bas_report_module
GET    /api/gst_bas_report_module/:id
PATCH  /api/gst_bas_report_module/:id
DELETE /api/gst_bas_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/gst_bas_report_module
```
...
```txt
GST_BAS_REPORT_MODULE_VIEWED
GST_BAS_REPORT_MODULE_CREATED
GST_BAS_REPORT_MODULE_UPDATED
GST_BAS_REPORT_MODULE_DELETED
GST_BAS_REPORT_MODULE_ACCESS_DENIED
GST_BAS_REPORT_MODULE_EXPORTED
```
