# 21. Review Request Module — Full Module Specification

## Main Purpose
The **Review Request Module** is responsible for post-job review links and follow-up automation in **Tradie Job Manager**.
...
```txt
GET    /api/review_request_module
POST   /api/review_request_module
GET    /api/review_request_module/:id
PATCH  /api/review_request_module/:id
DELETE /api/review_request_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/review_request_module
```
...
```txt
REVIEW_REQUEST_MODULE_VIEWED
REVIEW_REQUEST_MODULE_CREATED
REVIEW_REQUEST_MODULE_UPDATED
REVIEW_REQUEST_MODULE_DELETED
REVIEW_REQUEST_MODULE_ACCESS_DENIED
REVIEW_REQUEST_MODULE_EXPORTED
```
