# 27. Leave Management Module — Full Module Specification

## Main Purpose
The **Leave Management Module** is responsible for leave requests, approval and calendar blocking in **Tradie Job Manager**.
...
```txt
GET    /api/leave_management_module
POST   /api/leave_management_module
GET    /api/leave_management_module/:id
PATCH  /api/leave_management_module/:id
DELETE /api/leave_management_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/leave_management_module
```
...
```txt
LEAVE_MANAGEMENT_MODULE_VIEWED
LEAVE_MANAGEMENT_MODULE_CREATED
LEAVE_MANAGEMENT_MODULE_UPDATED
LEAVE_MANAGEMENT_MODULE_DELETED
LEAVE_MANAGEMENT_MODULE_ACCESS_DENIED
LEAVE_MANAGEMENT_MODULE_EXPORTED
```
