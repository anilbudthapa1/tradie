# 66. Recurring Invoice Module — Full Module Specification

## Main Purpose
The **Recurring Invoice Module** is responsible for recurring invoices for contracts and maintenance in **Tradie Job Manager**.
...
```txt
GET    /api/recurring_invoice_module
POST   /api/recurring_invoice_module
GET    /api/recurring_invoice_module/:id
PATCH  /api/recurring_invoice_module/:id
DELETE /api/recurring_invoice_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/recurring_invoice_module
```
...
```txt
RECURRING_INVOICE_MODULE_VIEWED
RECURRING_INVOICE_MODULE_CREATED
RECURRING_INVOICE_MODULE_UPDATED
RECURRING_INVOICE_MODULE_DELETED
RECURRING_INVOICE_MODULE_ACCESS_DENIED
RECURRING_INVOICE_MODULE_EXPORTED
```
