# 59. GST Calculator Module — Full Module Specification

## Main Purpose
The **GST Calculator Module** is responsible for calculate GST compliant amounts in **Tradie Job Manager**.
...
```txt
GET    /api/gst_calculator_module
POST   /api/gst_calculator_module
GET    /api/gst_calculator_module/:id
PATCH  /api/gst_calculator_module/:id
DELETE /api/gst_calculator_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/gst_calculator_module
```
...
```txt
GST_CALCULATOR_MODULE_VIEWED
GST_CALCULATOR_MODULE_CREATED
GST_CALCULATOR_MODULE_UPDATED
GST_CALCULATOR_MODULE_DELETED
GST_CALCULATOR_MODULE_ACCESS_DENIED
GST_CALCULATOR_MODULE_EXPORTED
```
