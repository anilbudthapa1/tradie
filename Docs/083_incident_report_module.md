# 83. Incident Report Module — Full Module Specification

## Main Purpose
The **Incident Report Module** is responsible for incident reports with evidence and escalation in **Tradie Job Manager**.
...
```txt
GET    /api/incident_report_module
POST   /api/incident_report_module
GET    /api/incident_report_module/:id
PATCH  /api/incident_report_module/:id
DELETE /api/incident_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/incident_report_module
```
...
```txt
INCIDENT_REPORT_MODULE_VIEWED
INCIDENT_REPORT_MODULE_CREATED
INCIDENT_REPORT_MODULE_UPDATED
INCIDENT_REPORT_MODULE_DELETED
INCIDENT_REPORT_MODULE_ACCESS_DENIED
INCIDENT_REPORT_MODULE_EXPORTED
```
