# 55. Quote Builder Module — Full Module Specification

## Main Purpose
The **Quote Builder Module** is responsible for line items, labour, materials, GST and discounts in **Tradie Job Manager**.
...
```txt
GET    /api/quote_builder_module
POST   /api/quote_builder_module
GET    /api/quote_builder_module/:id
PATCH  /api/quote_builder_module/:id
DELETE /api/quote_builder_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/quote_builder_module
```
...
```txt
QUOTE_BUILDER_MODULE_VIEWED
QUOTE_BUILDER_MODULE_CREATED
QUOTE_BUILDER_MODULE_UPDATED
QUOTE_BUILDER_MODULE_DELETED
QUOTE_BUILDER_MODULE_ACCESS_DENIED
QUOTE_BUILDER_MODULE_EXPORTED
```
