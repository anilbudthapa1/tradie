# 116. MYOB Integration Module — Full Module Specification

## Main Purpose
The **MYOB Integration Module** is responsible for sync finance data to MYOB later in **Tradie Job Manager**.
...
```txt
GET    /api/myob_integration_module
POST   /api/myob_integration_module
GET    /api/myob_integration_module/:id
PATCH  /api/myob_integration_module/:id
DELETE /api/myob_integration_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/myob_integration_module
```
...
```txt
MYOB_INTEGRATION_MODULE_VIEWED
MYOB_INTEGRATION_MODULE_CREATED
MYOB_INTEGRATION_MODULE_UPDATED
MYOB_INTEGRATION_MODULE_DELETED
MYOB_INTEGRATION_MODULE_ACCESS_DENIED
MYOB_INTEGRATION_MODULE_EXPORTED
```
