# 120. File Storage Module — Full Module Specification

## Main Purpose
The **File Storage Module** is responsible for secure S3-compatible file storage in **Tradie Job Manager**.
...
```txt
GET    /api/file_storage_module
POST   /api/file_storage_module
GET    /api/file_storage_module/:id
PATCH  /api/file_storage_module/:id
DELETE /api/file_storage_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/file_storage_module
```
...
```txt
FILE_STORAGE_MODULE_VIEWED
FILE_STORAGE_MODULE_CREATED
FILE_STORAGE_MODULE_UPDATED
FILE_STORAGE_MODULE_DELETED
FILE_STORAGE_MODULE_ACCESS_DENIED
FILE_STORAGE_MODULE_EXPORTED
```
