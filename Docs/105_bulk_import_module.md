# 105. Bulk Import Module — Full Module Specification

## Main Purpose
The **Bulk Import Module** is responsible for bulk CSV import for customers/jobs/items in **Tradie Job Manager**.
...
```txt
GET    /api/bulk_import_module
POST   /api/bulk_import_module
GET    /api/bulk_import_module/:id
PATCH  /api/bulk_import_module/:id
DELETE /api/bulk_import_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/bulk_import_module
```
...
```txt
BULK_IMPORT_MODULE_VIEWED
BULK_IMPORT_MODULE_CREATED
BULK_IMPORT_MODULE_UPDATED
BULK_IMPORT_MODULE_DELETED
BULK_IMPORT_MODULE_ACCESS_DENIED
BULK_IMPORT_MODULE_EXPORTED
```
