# 107. Audit Log Module — Full Module Specification

## Main Purpose
The **Audit Log Module** is responsible for immutable security/business audit logs in **Tradie Job Manager**.
...
```txt
GET    /api/audit_log_module
POST   /api/audit_log_module
GET    /api/audit_log_module/:id
PATCH  /api/audit_log_module/:id
DELETE /api/audit_log_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/audit_log_module
```
...
```txt
AUDIT_LOG_MODULE_VIEWED
AUDIT_LOG_MODULE_CREATED
AUDIT_LOG_MODULE_UPDATED
AUDIT_LOG_MODULE_DELETED
AUDIT_LOG_MODULE_ACCESS_DENIED
AUDIT_LOG_MODULE_EXPORTED
```
