# 103. CSV Export Module — Full Module Specification

## Main Purpose
The **CSV Export Module** is responsible for CSV exports for jobs/customers/finance in **Tradie Job Manager**.
...
```txt
GET    /api/csv_export_module
POST   /api/csv_export_module
GET    /api/csv_export_module/:id
PATCH  /api/csv_export_module/:id
DELETE /api/csv_export_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/csv_export_module
```
...
```txt
CSV_EXPORT_MODULE_VIEWED
CSV_EXPORT_MODULE_CREATED
CSV_EXPORT_MODULE_UPDATED
CSV_EXPORT_MODULE_DELETED
CSV_EXPORT_MODULE_ACCESS_DENIED
CSV_EXPORT_MODULE_EXPORTED
```
