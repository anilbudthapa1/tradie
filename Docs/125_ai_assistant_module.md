# 125. AI Assistant Module — Full Module Specification

## Main Purpose
The **AI Assistant Module** is responsible for optional assistant for drafting, summaries and workflow help in **Tradie Job Manager**.
...
```txt
GET    /api/ai_assistant_module
POST   /api/ai_assistant_module
GET    /api/ai_assistant_module/:id
PATCH  /api/ai_assistant_module/:id
DELETE /api/ai_assistant_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/ai_assistant_module
```
...
```txt
AI_ASSISTANT_MODULE_VIEWED
AI_ASSISTANT_MODULE_CREATED
AI_ASSISTANT_MODULE_UPDATED
AI_ASSISTANT_MODULE_DELETED
AI_ASSISTANT_MODULE_ACCESS_DENIED
AI_ASSISTANT_MODULE_EXPORTED
```
