# 36. Recurring Jobs Module — Full Module Specification

## Main Purpose
The **Recurring Jobs Module** is responsible for repeat jobs for maintenance/service contracts in **Tradie Job Manager**.
...
```txt
GET    /api/recurring_jobs_module
POST   /api/recurring_jobs_module
GET    /api/recurring_jobs_module/:id
PATCH  /api/recurring_jobs_module/:id
DELETE /api/recurring_jobs_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/recurring_jobs_module
```
...
```txt
RECURRING_JOBS_MODULE_VIEWED
RECURRING_JOBS_MODULE_CREATED
RECURRING_JOBS_MODULE_UPDATED
RECURRING_JOBS_MODULE_DELETED
RECURRING_JOBS_MODULE_ACCESS_DENIED
RECURRING_JOBS_MODULE_EXPORTED
```
