# 106. Bulk Update Module — Full Module Specification

## Main Purpose
The **Bulk Update Module** is responsible for bulk status/tag/assignment updates in **Tradie Job Manager**.
...
```txt
GET    /api/bulk_update_module
POST   /api/bulk_update_module
GET    /api/bulk_update_module/:id
PATCH  /api/bulk_update_module/:id
DELETE /api/bulk_update_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/bulk_update_module
```
...
```txt
BULK_UPDATE_MODULE_VIEWED
BULK_UPDATE_MODULE_CREATED
BULK_UPDATE_MODULE_UPDATED
BULK_UPDATE_MODULE_DELETED
BULK_UPDATE_MODULE_ACCESS_DENIED
BULK_UPDATE_MODULE_EXPORTED
```
