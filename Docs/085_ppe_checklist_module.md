# 85. PPE Checklist Module — Full Module Specification

## Main Purpose
The **PPE Checklist Module** is responsible for PPE issue/verification checklist in **Tradie Job Manager**.
...
```txt
GET    /api/ppe_checklist_module
POST   /api/ppe_checklist_module
GET    /api/ppe_checklist_module/:id
PATCH  /api/ppe_checklist_module/:id
DELETE /api/ppe_checklist_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/ppe_checklist_module
```
...
```txt
PPE_CHECKLIST_MODULE_VIEWED
PPE_CHECKLIST_MODULE_CREATED
PPE_CHECKLIST_MODULE_UPDATED
PPE_CHECKLIST_MODULE_DELETED
PPE_CHECKLIST_MODULE_ACCESS_DENIED
PPE_CHECKLIST_MODULE_EXPORTED
```
