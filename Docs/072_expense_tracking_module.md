# 72. Expense Tracking Module — Full Module Specification

## Main Purpose
The **Expense Tracking Module** is responsible for record operating expenses and bills in **Tradie Job Manager**.
...
```txt
GET    /api/expense_tracking_module
POST   /api/expense_tracking_module
GET    /api/expense_tracking_module/:id
PATCH  /api/expense_tracking_module/:id
DELETE /api/expense_tracking_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/expense_tracking_module
```
...
```txt
EXPENSE_TRACKING_MODULE_VIEWED
EXPENSE_TRACKING_MODULE_CREATED
EXPENSE_TRACKING_MODULE_UPDATED
EXPENSE_TRACKING_MODULE_DELETED
EXPENSE_TRACKING_MODULE_ACCESS_DENIED
EXPENSE_TRACKING_MODULE_EXPORTED
```
