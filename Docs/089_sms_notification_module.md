# 89. SMS Notification Module — Full Module Specification

## Main Purpose
The **SMS Notification Module** is responsible for send SMS reminders/ETA/alerts in **Tradie Job Manager**.
...
```txt
GET    /api/sms_notification_module
POST   /api/sms_notification_module
GET    /api/sms_notification_module/:id
PATCH  /api/sms_notification_module/:id
DELETE /api/sms_notification_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/sms_notification_module
```
...
```txt
SMS_NOTIFICATION_MODULE_VIEWED
SMS_NOTIFICATION_MODULE_CREATED
SMS_NOTIFICATION_MODULE_UPDATED
SMS_NOTIFICATION_MODULE_DELETED
SMS_NOTIFICATION_MODULE_ACCESS_DENIED
SMS_NOTIFICATION_MODULE_EXPORTED
```
