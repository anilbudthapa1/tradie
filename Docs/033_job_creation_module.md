# 33. Job Creation Module — Full Module Specification

## Main Purpose
The **Job Creation Module** is responsible for create jobs from scratch, quote, customer or recurring template in **Tradie Job Manager**.
...
```txt
GET    /api/job_creation_module
POST   /api/job_creation_module
GET    /api/job_creation_module/:id
PATCH  /api/job_creation_module/:id
DELETE /api/job_creation_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/job_creation_module
```
...
```txt
JOB_CREATION_MODULE_VIEWED
JOB_CREATION_MODULE_CREATED
JOB_CREATION_MODULE_UPDATED
JOB_CREATION_MODULE_DELETED
JOB_CREATION_MODULE_ACCESS_DENIED
JOB_CREATION_MODULE_EXPORTED
```
