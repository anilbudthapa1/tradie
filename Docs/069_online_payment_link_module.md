# 69. Online Payment Link Module — Full Module Specification

## Main Purpose
The **Online Payment Link Module** is responsible for secure payment links via Stripe/provider in **Tradie Job Manager**.
...
```txt
GET    /api/online_payment_link_module
POST   /api/online_payment_link_module
GET    /api/online_payment_link_module/:id
PATCH  /api/online_payment_link_module/:id
DELETE /api/online_payment_link_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/online_payment_link_module
```
...
```txt
ONLINE_PAYMENT_LINK_MODULE_VIEWED
ONLINE_PAYMENT_LINK_MODULE_CREATED
ONLINE_PAYMENT_LINK_MODULE_UPDATED
ONLINE_PAYMENT_LINK_MODULE_DELETED
ONLINE_PAYMENT_LINK_MODULE_ACCESS_DENIED
ONLINE_PAYMENT_LINK_MODULE_EXPORTED
```
