# 47. Conflict Detection Module — Full Module Specification

## Main Purpose
The **Conflict Detection Module** is responsible for detect double booking, travel conflicts and unavailable workers in **Tradie Job Manager**.
...
```txt
GET    /api/conflict_detection_module
POST   /api/conflict_detection_module
GET    /api/conflict_detection_module/:id
PATCH  /api/conflict_detection_module/:id
DELETE /api/conflict_detection_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/conflict_detection_module
```
...
```txt
CONFLICT_DETECTION_MODULE_VIEWED
CONFLICT_DETECTION_MODULE_CREATED
CONFLICT_DETECTION_MODULE_UPDATED
CONFLICT_DETECTION_MODULE_DELETED
CONFLICT_DETECTION_MODULE_ACCESS_DENIED
CONFLICT_DETECTION_MODULE_EXPORTED
```
