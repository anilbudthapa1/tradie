# 114. Stripe Integration Module — Full Module Specification

## Main Purpose
The **Stripe Integration Module** is responsible for subscription billing and payment links in **Tradie Job Manager**.
...
```txt
GET    /api/stripe_integration_module
POST   /api/stripe_integration_module
GET    /api/stripe_integration_module/:id
PATCH  /api/stripe_integration_module/:id
DELETE /api/stripe_integration_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/stripe_integration_module
```
...
```txt
STRIPE_INTEGRATION_MODULE_VIEWED
STRIPE_INTEGRATION_MODULE_CREATED
STRIPE_INTEGRATION_MODULE_UPDATED
STRIPE_INTEGRATION_MODULE_DELETED
STRIPE_INTEGRATION_MODULE_ACCESS_DENIED
STRIPE_INTEGRATION_MODULE_EXPORTED
```
