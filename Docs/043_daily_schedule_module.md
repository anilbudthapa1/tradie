# 43. Daily Schedule Module — Full Module Specification

## Main Purpose
The **Daily Schedule Module** is responsible for daily job schedule by worker/route/status in **Tradie Job Manager**.
...
```txt
GET    /api/daily_schedule_module
POST   /api/daily_schedule_module
GET    /api/daily_schedule_module/:id
PATCH  /api/daily_schedule_module/:id
DELETE /api/daily_schedule_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/daily_schedule_module
```
...
```txt
DAILY_SCHEDULE_MODULE_VIEWED
DAILY_SCHEDULE_MODULE_CREATED
DAILY_SCHEDULE_MODULE_UPDATED
DAILY_SCHEDULE_MODULE_DELETED
DAILY_SCHEDULE_MODULE_ACCESS_DENIED
DAILY_SCHEDULE_MODULE_EXPORTED
```
