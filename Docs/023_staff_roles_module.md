# 23. Staff Roles Module — Full Module Specification

## Main Purpose
The **Staff Roles Module** is responsible for role labels, responsibility mapping and future custom roles in **Tradie Job Manager**.
...
```txt
GET    /api/staff_roles_module
POST   /api/staff_roles_module
GET    /api/staff_roles_module/:id
PATCH  /api/staff_roles_module/:id
DELETE /api/staff_roles_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/staff_roles_module
```
...
```txt
STAFF_ROLES_MODULE_VIEWED
STAFF_ROLES_MODULE_CREATED
STAFF_ROLES_MODULE_UPDATED
STAFF_ROLES_MODULE_DELETED
STAFF_ROLES_MODULE_ACCESS_DENIED
STAFF_ROLES_MODULE_EXPORTED
```
