# 67. Payment Tracking Module — Full Module Specification

## Main Purpose
The **Payment Tracking Module** is responsible for track paid/unpaid/overdue/partial payments in **Tradie Job Manager**.
...
```txt
GET    /api/payment_tracking_module
POST   /api/payment_tracking_module
GET    /api/payment_tracking_module/:id
PATCH  /api/payment_tracking_module/:id
DELETE /api/payment_tracking_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/payment_tracking_module
```
...
```txt
PAYMENT_TRACKING_MODULE_VIEWED
PAYMENT_TRACKING_MODULE_CREATED
PAYMENT_TRACKING_MODULE_UPDATED
PAYMENT_TRACKING_MODULE_DELETED
PAYMENT_TRACKING_MODULE_ACCESS_DENIED
PAYMENT_TRACKING_MODULE_EXPORTED
```
