# 112. API Access Module — Full Module Specification

## Main Purpose
The **API Access Module** is responsible for API keys and external integrations access in **Tradie Job Manager**.
...
```txt
GET    /api/api_access_module
POST   /api/api_access_module
GET    /api/api_access_module/:id
PATCH  /api/api_access_module/:id
DELETE /api/api_access_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/api_access_module
```
...
```txt
API_ACCESS_MODULE_VIEWED
API_ACCESS_MODULE_CREATED
API_ACCESS_MODULE_UPDATED
API_ACCESS_MODULE_DELETED
API_ACCESS_MODULE_ACCESS_DENIED
API_ACCESS_MODULE_EXPORTED
```
