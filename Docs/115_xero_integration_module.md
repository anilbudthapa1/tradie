# 115. Xero Integration Module — Full Module Specification

## Main Purpose
The **Xero Integration Module** is responsible for sync invoices/bills/payments to Xero later in **Tradie Job Manager**.
...
```txt
GET    /api/xero_integration_module
POST   /api/xero_integration_module
GET    /api/xero_integration_module/:id
PATCH  /api/xero_integration_module/:id
DELETE /api/xero_integration_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/xero_integration_module
```
...
```txt
XERO_INTEGRATION_MODULE_VIEWED
XERO_INTEGRATION_MODULE_CREATED
XERO_INTEGRATION_MODULE_UPDATED
XERO_INTEGRATION_MODULE_DELETED
XERO_INTEGRATION_MODULE_ACCESS_DENIED
XERO_INTEGRATION_MODULE_EXPORTED
```
