# 87. Safety Sign-Off Module — Full Module Specification

## Main Purpose
The **Safety Sign-Off Module** is responsible for worker/customer safety sign-off records in **Tradie Job Manager**.
...
```txt
GET    /api/safety_sign_off_module
POST   /api/safety_sign_off_module
GET    /api/safety_sign_off_module/:id
PATCH  /api/safety_sign_off_module/:id
DELETE /api/safety_sign_off_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/safety_sign_off_module
```
...
```txt
SAFETY_SIGN_OFF_MODULE_VIEWED
SAFETY_SIGN_OFF_MODULE_CREATED
SAFETY_SIGN_OFF_MODULE_UPDATED
SAFETY_SIGN_OFF_MODULE_DELETED
SAFETY_SIGN_OFF_MODULE_ACCESS_DENIED
SAFETY_SIGN_OFF_MODULE_EXPORTED
```
