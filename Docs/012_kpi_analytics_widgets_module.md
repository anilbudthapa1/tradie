# 12. KPI Analytics Widgets Module — Full Module Specification

## Main Purpose
The **KPI Analytics Widgets Module** is responsible for revenue, jobs, quotes, invoices, payroll and safety KPI cards in **Tradie Job Manager**.
...
```txt
KPIWidgetService, MetricsQueryService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/kpi_analytics_widgets_module
POST   /api/kpi_analytics_widgets_module
GET    /api/kpi_analytics_widgets_module/:id
PATCH  /api/kpi_analytics_widgets_module/:id
DELETE /api/kpi_analytics_widgets_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/kpi_analytics_widgets_module
```
...
```txt
KPI_ANALYTICS_WIDGETS_MODULE_VIEWED
KPI_ANALYTICS_WIDGETS_MODULE_CREATED
KPI_ANALYTICS_WIDGETS_MODULE_UPDATED
KPI_ANALYTICS_WIDGETS_MODULE_DELETED
KPI_ANALYTICS_WIDGETS_MODULE_ACCESS_DENIED
KPI_ANALYTICS_WIDGETS_MODULE_EXPORTED
```
