# 25. Time Tracking Module — Full Module Specification

## Main Purpose
The **Time Tracking Module** is responsible for timesheets, job time, break time and approvals in **Tradie Job Manager**.
...
## Core Permissions
```txt
time.view, time.create, time.approve
```
...
```txt
GET    /api/time_tracking_module
POST   /api/time_tracking_module
GET    /api/time_tracking_module/:id
PATCH  /api/time_tracking_module/:id
DELETE /api/time_tracking_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/time_tracking_module
```
...
```txt
TIME_TRACKING_MODULE_VIEWED
TIME_TRACKING_MODULE_CREATED
TIME_TRACKING_MODULE_UPDATED
TIME_TRACKING_MODULE_DELETED
TIME_TRACKING_MODULE_ACCESS_DENIED
TIME_TRACKING_MODULE_EXPORTED
```
