# 13. Task Reminder Module — Full Module Specification

## Main Purpose
The **Task Reminder Module** is responsible for task reminders for jobs, invoices, safety, licences and follow-ups in **Tradie Job Manager**.
...
```txt
ReminderService, SchedulerService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/task_reminder_module
POST   /api/task_reminder_module
GET    /api/task_reminder_module/:id
PATCH  /api/task_reminder_module/:id
DELETE /api/task_reminder_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/task_reminder_module
```
...
```txt
TASK_REMINDER_MODULE_VIEWED
TASK_REMINDER_MODULE_CREATED
TASK_REMINDER_MODULE_UPDATED
TASK_REMINDER_MODULE_DELETED
TASK_REMINDER_MODULE_ACCESS_DENIED
TASK_REMINDER_MODULE_EXPORTED
```
