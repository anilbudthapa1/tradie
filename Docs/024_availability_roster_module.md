# 24. Availability Roster Module — Full Module Specification

## Main Purpose
The **Availability Roster Module** is responsible for worker availability, leave blocks and roster planning in **Tradie Job Manager**.
...
```txt
GET    /api/availability_roster_module
POST   /api/availability_roster_module
GET    /api/availability_roster_module/:id
PATCH  /api/availability_roster_module/:id
DELETE /api/availability_roster_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/availability_roster_module
```
...
```txt
AVAILABILITY_ROSTER_MODULE_VIEWED
AVAILABILITY_ROSTER_MODULE_CREATED
AVAILABILITY_ROSTER_MODULE_UPDATED
AVAILABILITY_ROSTER_MODULE_DELETED
AVAILABILITY_ROSTER_MODULE_ACCESS_DENIED
AVAILABILITY_ROSTER_MODULE_EXPORTED
```
