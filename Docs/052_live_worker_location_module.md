# 52. Live Worker Location Module — Full Module Specification

## Main Purpose
The **Live Worker Location Module** is responsible for optional live worker tracking during work hours in **Tradie Job Manager**.
...
```txt
GET    /api/live_worker_location_module
POST   /api/live_worker_location_module
GET    /api/live_worker_location_module/:id
PATCH  /api/live_worker_location_module/:id
DELETE /api/live_worker_location_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/live_worker_location_module
```
...
```txt
LIVE_WORKER_LOCATION_MODULE_VIEWED
LIVE_WORKER_LOCATION_MODULE_CREATED
LIVE_WORKER_LOCATION_MODULE_UPDATED
LIVE_WORKER_LOCATION_MODULE_DELETED
LIVE_WORKER_LOCATION_MODULE_ACCESS_DENIED
LIVE_WORKER_LOCATION_MODULE_EXPORTED
```
