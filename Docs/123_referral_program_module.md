# 123. Referral Program Module — Full Module Specification

## Main Purpose
The **Referral Program Module** is responsible for referrals, invites and rewards in **Tradie Job Manager**.
...
```txt
GET    /api/referral_program_module
POST   /api/referral_program_module
GET    /api/referral_program_module/:id
PATCH  /api/referral_program_module/:id
DELETE /api/referral_program_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/referral_program_module
```
...
```txt
REFERRAL_PROGRAM_MODULE_VIEWED
REFERRAL_PROGRAM_MODULE_CREATED
REFERRAL_PROGRAM_MODULE_UPDATED
REFERRAL_PROGRAM_MODULE_DELETED
REFERRAL_PROGRAM_MODULE_ACCESS_DENIED
REFERRAL_PROGRAM_MODULE_EXPORTED
```
