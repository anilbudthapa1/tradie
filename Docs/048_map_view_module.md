# 48. Map View Module — Full Module Specification

## Main Purpose
The **Map View Module** is responsible for display jobs/workers/customers on map in **Tradie Job Manager**.
...
```txt
GET    /api/map_view_module
POST   /api/map_view_module
GET    /api/map_view_module/:id
PATCH  /api/map_view_module/:id
DELETE /api/map_view_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/map_view_module
```
...
```txt
MAP_VIEW_MODULE_VIEWED
MAP_VIEW_MODULE_CREATED
MAP_VIEW_MODULE_UPDATED
MAP_VIEW_MODULE_DELETED
MAP_VIEW_MODULE_ACCESS_DENIED
MAP_VIEW_MODULE_EXPORTED
```
