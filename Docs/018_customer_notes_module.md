# 18. Customer Notes Module — Full Module Specification

## Main Purpose
The **Customer Notes Module** is responsible for private/internal/customer notes with audit history in **Tradie Job Manager**.
...
```txt
GET    /api/customer_notes_module
POST   /api/customer_notes_module
GET    /api/customer_notes_module/:id
PATCH  /api/customer_notes_module/:id
DELETE /api/customer_notes_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/customer_notes_module
```
...
```txt
CUSTOMER_NOTES_MODULE_VIEWED
CUSTOMER_NOTES_MODULE_CREATED
CUSTOMER_NOTES_MODULE_UPDATED
CUSTOMER_NOTES_MODULE_DELETED
CUSTOMER_NOTES_MODULE_ACCESS_DENIED
CUSTOMER_NOTES_MODULE_EXPORTED
```
