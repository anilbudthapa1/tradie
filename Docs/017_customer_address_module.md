# 17. Customer Address Module — Full Module Specification

## Main Purpose
The **Customer Address Module** is responsible for multiple service/billing/postal addresses per customer in **Tradie Job Manager**.
...
```txt
GET    /api/customer_address_module
POST   /api/customer_address_module
GET    /api/customer_address_module/:id
PATCH  /api/customer_address_module/:id
DELETE /api/customer_address_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/customer_address_module
```
...
```txt
CUSTOMER_ADDRESS_MODULE_VIEWED
CUSTOMER_ADDRESS_MODULE_CREATED
CUSTOMER_ADDRESS_MODULE_UPDATED
CUSTOMER_ADDRESS_MODULE_DELETED
CUSTOMER_ADDRESS_MODULE_ACCESS_DENIED
CUSTOMER_ADDRESS_MODULE_EXPORTED
```
