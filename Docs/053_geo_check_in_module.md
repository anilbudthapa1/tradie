# 53. Geo Check-In Module — Full Module Specification

## Main Purpose
The **Geo Check-In Module** is responsible for validate check-in near job location in **Tradie Job Manager**.
...
```txt
GET    /api/geo_check_in_module
POST   /api/geo_check_in_module
GET    /api/geo_check_in_module/:id
PATCH  /api/geo_check_in_module/:id
DELETE /api/geo_check_in_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/geo_check_in_module
```
...
```txt
GEO_CHECK_IN_MODULE_VIEWED
GEO_CHECK_IN_MODULE_CREATED
GEO_CHECK_IN_MODULE_UPDATED
GEO_CHECK_IN_MODULE_DELETED
GEO_CHECK_IN_MODULE_ACCESS_DENIED
GEO_CHECK_IN_MODULE_EXPORTED
```
