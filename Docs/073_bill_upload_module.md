# 73. Bill Upload Module — Full Module Specification

## Main Purpose
The **Bill Upload Module** is responsible for upload supplier bills and receipts in **Tradie Job Manager**.
...
```txt
GET    /api/bill_upload_module
POST   /api/bill_upload_module
GET    /api/bill_upload_module/:id
PATCH  /api/bill_upload_module/:id
DELETE /api/bill_upload_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/bill_upload_module
```
...
```txt
BILL_UPLOAD_MODULE_VIEWED
BILL_UPLOAD_MODULE_CREATED
BILL_UPLOAD_MODULE_UPDATED
BILL_UPLOAD_MODULE_DELETED
BILL_UPLOAD_MODULE_ACCESS_DENIED
BILL_UPLOAD_MODULE_EXPORTED
```
