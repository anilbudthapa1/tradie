# 121. Mobile App Sync Module — Full Module Specification

## Main Purpose
The **Mobile App Sync Module** is responsible for offline-first mobile sync later in **Tradie Job Manager**.
...
```txt
GET    /api/mobile_app_sync_module
POST   /api/mobile_app_sync_module
GET    /api/mobile_app_sync_module/:id
PATCH  /api/mobile_app_sync_module/:id
DELETE /api/mobile_app_sync_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/mobile_app_sync_module
```
...
```txt
MOBILE_APP_SYNC_MODULE_VIEWED
MOBILE_APP_SYNC_MODULE_CREATED
MOBILE_APP_SYNC_MODULE_UPDATED
MOBILE_APP_SYNC_MODULE_DELETED
MOBILE_APP_SYNC_MODULE_ACCESS_DENIED
MOBILE_APP_SYNC_MODULE_EXPORTED
```
