# 70. Receipt Generator Module — Full Module Specification

## Main Purpose
The **Receipt Generator Module** is responsible for generate receipts after payment in **Tradie Job Manager**.
...
```txt
GET    /api/receipt_generator_module
POST   /api/receipt_generator_module
GET    /api/receipt_generator_module/:id
PATCH  /api/receipt_generator_module/:id
DELETE /api/receipt_generator_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/receipt_generator_module
```
...
```txt
RECEIPT_GENERATOR_MODULE_VIEWED
RECEIPT_GENERATOR_MODULE_CREATED
RECEIPT_GENERATOR_MODULE_UPDATED
RECEIPT_GENERATOR_MODULE_DELETED
RECEIPT_GENERATOR_MODULE_ACCESS_DENIED
RECEIPT_GENERATOR_MODULE_EXPORTED
```
