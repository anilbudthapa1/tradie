# 16. Customer Contact Module — Full Module Specification

## Main Purpose
The **Customer Contact Module** is responsible for emails, phones, communication preferences and contact history in **Tradie Job Manager**.
...
```txt
GET    /api/customer_contact_module
POST   /api/customer_contact_module
GET    /api/customer_contact_module/:id
PATCH  /api/customer_contact_module/:id
DELETE /api/customer_contact_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/customer_contact_module
```
...
```txt
CUSTOMER_CONTACT_MODULE_VIEWED
CUSTOMER_CONTACT_MODULE_CREATED
CUSTOMER_CONTACT_MODULE_UPDATED
CUSTOMER_CONTACT_MODULE_DELETED
CUSTOMER_CONTACT_MODULE_ACCESS_DENIED
CUSTOMER_CONTACT_MODULE_EXPORTED
```
