# 37. Priority Jobs Module — Full Module Specification

## Main Purpose
The **Priority Jobs Module** is responsible for urgent/high/normal priority tagging and queues in **Tradie Job Manager**.
...
```txt
GET    /api/priority_jobs_module
POST   /api/priority_jobs_module
GET    /api/priority_jobs_module/:id
PATCH  /api/priority_jobs_module/:id
DELETE /api/priority_jobs_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/priority_jobs_module
```
...
```txt
PRIORITY_JOBS_MODULE_VIEWED
PRIORITY_JOBS_MODULE_CREATED
PRIORITY_JOBS_MODULE_UPDATED
PRIORITY_JOBS_MODULE_DELETED
PRIORITY_JOBS_MODULE_ACCESS_DENIED
PRIORITY_JOBS_MODULE_EXPORTED
```
