# 49. Route Planning Module — Full Module Specification

## Main Purpose
The **Route Planning Module** is responsible for optimise job order and route travel in **Tradie Job Manager**.
...
```txt
GET    /api/route_planning_module
POST   /api/route_planning_module
GET    /api/route_planning_module/:id
PATCH  /api/route_planning_module/:id
DELETE /api/route_planning_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/route_planning_module
```
...
```txt
ROUTE_PLANNING_MODULE_VIEWED
ROUTE_PLANNING_MODULE_CREATED
ROUTE_PLANNING_MODULE_UPDATED
ROUTE_PLANNING_MODULE_DELETED
ROUTE_PLANNING_MODULE_ACCESS_DENIED
ROUTE_PLANNING_MODULE_EXPORTED
```
