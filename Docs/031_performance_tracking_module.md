# 31. Performance Tracking Module — Full Module Specification

## Main Purpose
The **Performance Tracking Module** is responsible for worker job completion, punctuality and productivity metrics in **Tradie Job Manager**.
...
```txt
GET    /api/performance_tracking_module
POST   /api/performance_tracking_module
GET    /api/performance_tracking_module/:id
PATCH  /api/performance_tracking_module/:id
DELETE /api/performance_tracking_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/performance_tracking_module
```
...
```txt
PERFORMANCE_TRACKING_MODULE_VIEWED
PERFORMANCE_TRACKING_MODULE_CREATED
PERFORMANCE_TRACKING_MODULE_UPDATED
PERFORMANCE_TRACKING_MODULE_DELETED
PERFORMANCE_TRACKING_MODULE_ACCESS_DENIED
PERFORMANCE_TRACKING_MODULE_EXPORTED
```
