# 38. Job Notes Module — Full Module Specification

## Main Purpose
The **Job Notes Module** is responsible for job notes, internal comments and field updates in **Tradie Job Manager**.
...
## Core Permissions
```txt
job_notes.create, job_notes.view
```
...
```txt
GET    /api/job_notes_module
POST   /api/job_notes_module
GET    /api/job_notes_module/:id
PATCH  /api/job_notes_module/:id
DELETE /api/job_notes_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/job_notes_module
```
...
```txt
JOB_NOTES_MODULE_VIEWED
JOB_NOTES_MODULE_CREATED
JOB_NOTES_MODULE_UPDATED
JOB_NOTES_MODULE_DELETED
JOB_NOTES_MODULE_ACCESS_DENIED
JOB_NOTES_MODULE_EXPORTED
```
