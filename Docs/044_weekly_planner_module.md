# 44. Weekly Planner Module — Full Module Specification

## Main Purpose
The **Weekly Planner Module** is responsible for weekly planning board and capacity view in **Tradie Job Manager**.
...
```txt
GET    /api/weekly_planner_module
POST   /api/weekly_planner_module
GET    /api/weekly_planner_module/:id
PATCH  /api/weekly_planner_module/:id
DELETE /api/weekly_planner_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/weekly_planner_module
```
...
```txt
WEEKLY_PLANNER_MODULE_VIEWED
WEEKLY_PLANNER_MODULE_CREATED
WEEKLY_PLANNER_MODULE_UPDATED
WEEKLY_PLANNER_MODULE_DELETED
WEEKLY_PLANNER_MODULE_ACCESS_DENIED
WEEKLY_PLANNER_MODULE_EXPORTED
```
