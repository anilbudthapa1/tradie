# 29. Payslip Generator Module — Full Module Specification

## Main Purpose
The **Payslip Generator Module** is responsible for secure payslip PDF generation and employee access in **Tradie Job Manager**.
...
## Core Permissions
```txt
payslips.view_own, payslips.generate
```

## Suggested Go Services
```txt
PayslipService, PDFService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/payslip_generator_module
POST   /api/payslip_generator_module
GET    /api/payslip_generator_module/:id
PATCH  /api/payslip_generator_module/:id
DELETE /api/payslip_generator_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/payslip_generator_module
```
...
```txt
PAYSLIP_GENERATOR_MODULE_VIEWED
PAYSLIP_GENERATOR_MODULE_CREATED
PAYSLIP_GENERATOR_MODULE_UPDATED
PAYSLIP_GENERATOR_MODULE_DELETED
PAYSLIP_GENERATOR_MODULE_ACCESS_DENIED
PAYSLIP_GENERATOR_MODULE_EXPORTED
```
