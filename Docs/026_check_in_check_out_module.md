# 26. Check-In Check-Out Module — Full Module Specification

## Main Purpose
The **Check-In Check-Out Module** is responsible for geo/time check-in/out for assigned jobs in **Tradie Job Manager**.
...
```txt
GET    /api/check_in_check_out_module
POST   /api/check_in_check_out_module
GET    /api/check_in_check_out_module/:id
PATCH  /api/check_in_check_out_module/:id
DELETE /api/check_in_check_out_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/check_in_check_out_module
```
...
```txt
CHECK_IN_CHECK_OUT_MODULE_VIEWED
CHECK_IN_CHECK_OUT_MODULE_CREATED
CHECK_IN_CHECK_OUT_MODULE_UPDATED
CHECK_IN_CHECK_OUT_MODULE_DELETED
CHECK_IN_CHECK_OUT_MODULE_ACCESS_DENIED
CHECK_IN_CHECK_OUT_MODULE_EXPORTED
```
