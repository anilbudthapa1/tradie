# 39. Before After Photos Module — Full Module Specification

## Main Purpose
The **Before After Photos Module** is responsible for before/after job photos and evidence gallery in **Tradie Job Manager**.
...
## Core Permissions
```txt
job_photos.upload, job_photos.view
```

## Suggested Go Services
```txt
JobPhotoService, FileService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/before_after_photos_module
POST   /api/before_after_photos_module
GET    /api/before_after_photos_module/:id
PATCH  /api/before_after_photos_module/:id
DELETE /api/before_after_photos_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/before_after_photos_module
```
...
```txt
BEFORE_AFTER_PHOTOS_MODULE_VIEWED
BEFORE_AFTER_PHOTOS_MODULE_CREATED
BEFORE_AFTER_PHOTOS_MODULE_UPDATED
BEFORE_AFTER_PHOTOS_MODULE_DELETED
BEFORE_AFTER_PHOTOS_MODULE_ACCESS_DENIED
BEFORE_AFTER_PHOTOS_MODULE_EXPORTED
```
