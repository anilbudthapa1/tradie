# 71. Refund Credit Note Module — Full Module Specification

## Main Purpose
The **Refund Credit Note Module** is responsible for refunds and credit notes with audit in **Tradie Job Manager**.
...
```txt
GET    /api/refund_credit_note_module
POST   /api/refund_credit_note_module
GET    /api/refund_credit_note_module/:id
PATCH  /api/refund_credit_note_module/:id
DELETE /api/refund_credit_note_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/refund_credit_note_module
```
...
```txt
REFUND_CREDIT_NOTE_MODULE_VIEWED
REFUND_CREDIT_NOTE_MODULE_CREATED
REFUND_CREDIT_NOTE_MODULE_UPDATED
REFUND_CREDIT_NOTE_MODULE_DELETED
REFUND_CREDIT_NOTE_MODULE_ACCESS_DENIED
REFUND_CREDIT_NOTE_MODULE_EXPORTED
```
