# 46. Reschedule Module — Full Module Specification

## Main Purpose
The **Reschedule Module** is responsible for safe rescheduling with notifications and audit in **Tradie Job Manager**.
...
```txt
GET    /api/reschedule_module
POST   /api/reschedule_module
GET    /api/reschedule_module/:id
PATCH  /api/reschedule_module/:id
DELETE /api/reschedule_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/reschedule_module
```
...
```txt
RESCHEDULE_MODULE_VIEWED
RESCHEDULE_MODULE_CREATED
RESCHEDULE_MODULE_UPDATED
RESCHEDULE_MODULE_DELETED
RESCHEDULE_MODULE_ACCESS_DENIED
RESCHEDULE_MODULE_EXPORTED
```
