# 82. Risk Assessment Module — Full Module Specification

## Main Purpose
The **Risk Assessment Module** is responsible for job/site risk assessment forms in **Tradie Job Manager**.
...
```txt
GET    /api/risk_assessment_module
POST   /api/risk_assessment_module
GET    /api/risk_assessment_module/:id
PATCH  /api/risk_assessment_module/:id
DELETE /api/risk_assessment_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/risk_assessment_module
```
...
```txt
RISK_ASSESSMENT_MODULE_VIEWED
RISK_ASSESSMENT_MODULE_CREATED
RISK_ASSESSMENT_MODULE_UPDATED
RISK_ASSESSMENT_MODULE_DELETED
RISK_ASSESSMENT_MODULE_ACCESS_DENIED
RISK_ASSESSMENT_MODULE_EXPORTED
```
