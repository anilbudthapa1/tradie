# 65. Invoice Generator Module — Full Module Specification

## Main Purpose
The **Invoice Generator Module** is responsible for generate invoices from jobs/quotes in **Tradie Job Manager**.
...
```txt
GET    /api/invoice_generator_module
POST   /api/invoice_generator_module
GET    /api/invoice_generator_module/:id
PATCH  /api/invoice_generator_module/:id
DELETE /api/invoice_generator_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/invoice_generator_module
```
...
```txt
INVOICE_GENERATOR_MODULE_VIEWED
INVOICE_GENERATOR_MODULE_CREATED
INVOICE_GENERATOR_MODULE_UPDATED
INVOICE_GENERATOR_MODULE_DELETED
INVOICE_GENERATOR_MODULE_ACCESS_DENIED
INVOICE_GENERATOR_MODULE_EXPORTED
```
