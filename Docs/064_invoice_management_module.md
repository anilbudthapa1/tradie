# 64. Invoice Management Module — Full Module Specification

## Main Purpose
The **Invoice Management Module** is responsible for invoice lifecycle, statuses and invoice list in **Tradie Job Manager**.
...
```txt
GET    /api/invoice_management_module
POST   /api/invoice_management_module
GET    /api/invoice_management_module/:id
PATCH  /api/invoice_management_module/:id
DELETE /api/invoice_management_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/invoice_management_module
```
...
```txt
INVOICE_MANAGEMENT_MODULE_VIEWED
INVOICE_MANAGEMENT_MODULE_CREATED
INVOICE_MANAGEMENT_MODULE_UPDATED
INVOICE_MANAGEMENT_MODULE_DELETED
INVOICE_MANAGEMENT_MODULE_ACCESS_DENIED
INVOICE_MANAGEMENT_MODULE_EXPORTED
```
