# 19. Customer History Module — Full Module Specification

## Main Purpose
The **Customer History Module** is responsible for jobs, quotes, invoices, payments and notes history in **Tradie Job Manager**.
...
```txt
GET    /api/customer_history_module
POST   /api/customer_history_module
GET    /api/customer_history_module/:id
PATCH  /api/customer_history_module/:id
DELETE /api/customer_history_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/customer_history_module
```
...
```txt
CUSTOMER_HISTORY_MODULE_VIEWED
CUSTOMER_HISTORY_MODULE_CREATED
CUSTOMER_HISTORY_MODULE_UPDATED
CUSTOMER_HISTORY_MODULE_DELETED
CUSTOMER_HISTORY_MODULE_ACCESS_DENIED
CUSTOMER_HISTORY_MODULE_EXPORTED
```
