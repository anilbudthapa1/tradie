# 109. 2FA Module — Full Module Specification

## Main Purpose
The **2FA Module** is responsible for TOTP/passkey/backup-code MFA in **Tradie Job Manager**.
...
GET    /api/2fa_module
POST   /api/2fa_module
GET    /api/2fa_module/:id
PATCH  /api/2fa_module/:id
DELETE /api/2fa_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/2fa_module
```
