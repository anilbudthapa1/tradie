# 98. Unpaid Invoice Report Module — Full Module Specification

## Main Purpose
The **Unpaid Invoice Report Module** is responsible for aged receivables and unpaid invoices in **Tradie Job Manager**.
...
## Core Permissions
```txt
reports.unpaid_invoices.view
```

## Suggested Go Services
```txt
UnpaidInvoiceReportService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/unpaid_invoice_report_module
POST   /api/unpaid_invoice_report_module
GET    /api/unpaid_invoice_report_module/:id
PATCH  /api/unpaid_invoice_report_module/:id
DELETE /api/unpaid_invoice_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/unpaid_invoice_report_module
```
...
```txt
UNPAID_INVOICE_REPORT_MODULE_VIEWED
UNPAID_INVOICE_REPORT_MODULE_CREATED
UNPAID_INVOICE_REPORT_MODULE_UPDATED
UNPAID_INVOICE_REPORT_MODULE_DELETED
UNPAID_INVOICE_REPORT_MODULE_ACCESS_DENIED
UNPAID_INVOICE_REPORT_MODULE_EXPORTED
```
