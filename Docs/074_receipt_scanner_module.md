# 74. Receipt Scanner Module — Full Module Specification

## Main Purpose
The **Receipt Scanner Module** is responsible for OCR/metadata extraction for receipts later in **Tradie Job Manager**.
...
```txt
GET    /api/receipt_scanner_module
POST   /api/receipt_scanner_module
GET    /api/receipt_scanner_module/:id
PATCH  /api/receipt_scanner_module/:id
DELETE /api/receipt_scanner_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/receipt_scanner_module
```
...
```txt
RECEIPT_SCANNER_MODULE_VIEWED
RECEIPT_SCANNER_MODULE_CREATED
RECEIPT_SCANNER_MODULE_UPDATED
RECEIPT_SCANNER_MODULE_DELETED
RECEIPT_SCANNER_MODULE_ACCESS_DENIED
RECEIPT_SCANNER_MODULE_EXPORTED
```
