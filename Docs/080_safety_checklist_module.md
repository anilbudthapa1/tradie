# 80. Safety Checklist Module — Full Module Specification

## Main Purpose
The **Safety Checklist Module** is responsible for job/site safety checklist workflows in **Tradie Job Manager**.
...
```txt
GET    /api/safety_checklist_module
POST   /api/safety_checklist_module
GET    /api/safety_checklist_module/:id
PATCH  /api/safety_checklist_module/:id
DELETE /api/safety_checklist_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/safety_checklist_module
```
...
```txt
SAFETY_CHECKLIST_MODULE_VIEWED
SAFETY_CHECKLIST_MODULE_CREATED
SAFETY_CHECKLIST_MODULE_UPDATED
SAFETY_CHECKLIST_MODULE_DELETED
SAFETY_CHECKLIST_MODULE_ACCESS_DENIED
SAFETY_CHECKLIST_MODULE_EXPORTED
```
