# 68. Overdue Reminder Module — Full Module Specification

## Main Purpose
The **Overdue Reminder Module** is responsible for automated invoice overdue reminders in **Tradie Job Manager**.
...
```txt
GET    /api/overdue_reminder_module
POST   /api/overdue_reminder_module
GET    /api/overdue_reminder_module/:id
PATCH  /api/overdue_reminder_module/:id
DELETE /api/overdue_reminder_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/overdue_reminder_module
```
...
```txt
OVERDUE_REMINDER_MODULE_VIEWED
OVERDUE_REMINDER_MODULE_CREATED
OVERDUE_REMINDER_MODULE_UPDATED
OVERDUE_REMINDER_MODULE_DELETED
OVERDUE_REMINDER_MODULE_ACCESS_DENIED
OVERDUE_REMINDER_MODULE_EXPORTED
```
