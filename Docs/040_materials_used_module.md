# 40. Materials Used Module — Full Module Specification

## Main Purpose
The **Materials Used Module** is responsible for materials/labour consumption per job in **Tradie Job Manager**.
...
## Core Permissions
```txt
materials.used.manage
```

## Suggested Go Services
```txt
MaterialsUsedService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/materials_used_module
POST   /api/materials_used_module
GET    /api/materials_used_module/:id
PATCH  /api/materials_used_module/:id
DELETE /api/materials_used_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/materials_used_module
```
...
```txt
MATERIALS_USED_MODULE_VIEWED
MATERIALS_USED_MODULE_CREATED
MATERIALS_USED_MODULE_UPDATED
MATERIALS_USED_MODULE_DELETED
MATERIALS_USED_MODULE_ACCESS_DENIED
MATERIALS_USED_MODULE_EXPORTED
```
