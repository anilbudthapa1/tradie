# 124. Marketplace Leads Module — Full Module Specification

## Main Purpose
The **Marketplace Leads Module** is responsible for service request marketplace leads later in **Tradie Job Manager**.
...
## Core Permissions
```txt
marketplace.leads.view
```

## Suggested Go Services
```txt
MarketplaceLeadService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/marketplace_leads_module
POST   /api/marketplace_leads_module
GET    /api/marketplace_leads_module/:id
PATCH  /api/marketplace_leads_module/:id
DELETE /api/marketplace_leads_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/marketplace_leads_module
```
...
```txt
MARKETPLACE_LEADS_MODULE_VIEWED
MARKETPLACE_LEADS_MODULE_CREATED
MARKETPLACE_LEADS_MODULE_UPDATED
MARKETPLACE_LEADS_MODULE_DELETED
MARKETPLACE_LEADS_MODULE_ACCESS_DENIED
MARKETPLACE_LEADS_MODULE_EXPORTED
```
