# 104. PDF Export Module — Full Module Specification

## Main Purpose
The **PDF Export Module** is responsible for PDF reports, invoices, quotes and payslips in **Tradie Job Manager**.
...
```txt
GET    /api/pdf_export_module
POST   /api/pdf_export_module
GET    /api/pdf_export_module/:id
PATCH  /api/pdf_export_module/:id
DELETE /api/pdf_export_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/pdf_export_module
```
...
```txt
PDF_EXPORT_MODULE_VIEWED
PDF_EXPORT_MODULE_CREATED
PDF_EXPORT_MODULE_UPDATED
PDF_EXPORT_MODULE_DELETED
PDF_EXPORT_MODULE_ACCESS_DENIED
PDF_EXPORT_MODULE_EXPORTED
```
