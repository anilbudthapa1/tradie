# 111. Backup Restore Module — Full Module Specification

## Main Purpose
The **Backup Restore Module** is responsible for tenant-safe backups, export and restore in **Tradie Job Manager**.
...
```txt
GET    /api/backup_restore_module
POST   /api/backup_restore_module
GET    /api/backup_restore_module/:id
PATCH  /api/backup_restore_module/:id
DELETE /api/backup_restore_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/backup_restore_module
```
...
```txt
BACKUP_RESTORE_MODULE_VIEWED
BACKUP_RESTORE_MODULE_CREATED
BACKUP_RESTORE_MODULE_UPDATED
BACKUP_RESTORE_MODULE_DELETED
BACKUP_RESTORE_MODULE_ACCESS_DENIED
BACKUP_RESTORE_MODULE_EXPORTED
```
