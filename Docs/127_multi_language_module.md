# 127. Multi Language Module — Full Module Specification

## Main Purpose
The **Multi Language Module** is responsible for localisation and translated templates in **Tradie Job Manager**.
...
```txt
GET    /api/multi_language_module
POST   /api/multi_language_module
GET    /api/multi_language_module/:id
PATCH  /api/multi_language_module/:id
DELETE /api/multi_language_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/multi_language_module
```
...
```txt
MULTI_LANGUAGE_MODULE_VIEWED
MULTI_LANGUAGE_MODULE_CREATED
MULTI_LANGUAGE_MODULE_UPDATED
MULTI_LANGUAGE_MODULE_DELETED
MULTI_LANGUAGE_MODULE_ACCESS_DENIED
MULTI_LANGUAGE_MODULE_EXPORTED
```
