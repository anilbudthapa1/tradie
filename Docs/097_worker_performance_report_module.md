# 97. Worker Performance Report Module — Full Module Specification

## Main Purpose
The **Worker Performance Report Module** is responsible for worker productivity and punctuality reports in **Tradie Job Manager**.
...
## Core Permissions
```txt
reports.worker_performance.view
```

## Suggested Go Services
```txt
WorkerPerformanceReportService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/worker_performance_report_module
POST   /api/worker_performance_report_module
GET    /api/worker_performance_report_module/:id
PATCH  /api/worker_performance_report_module/:id
DELETE /api/worker_performance_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/worker_performance_report_module
```
...
```txt
WORKER_PERFORMANCE_REPORT_MODULE_VIEWED
WORKER_PERFORMANCE_REPORT_MODULE_CREATED
WORKER_PERFORMANCE_REPORT_MODULE_UPDATED
WORKER_PERFORMANCE_REPORT_MODULE_DELETED
WORKER_PERFORMANCE_REPORT_MODULE_ACCESS_DENIED
WORKER_PERFORMANCE_REPORT_MODULE_EXPORTED
```
