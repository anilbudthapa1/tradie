# 34. Job Assignment Module — Full Module Specification

## Main Purpose
The **Job Assignment Module** is responsible for assign/reassign workers and resources to jobs in **Tradie Job Manager**.
...
```txt
GET    /api/job_assignment_module
POST   /api/job_assignment_module
GET    /api/job_assignment_module/:id
PATCH  /api/job_assignment_module/:id
DELETE /api/job_assignment_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/job_assignment_module
```
...
```txt
JOB_ASSIGNMENT_MODULE_VIEWED
JOB_ASSIGNMENT_MODULE_CREATED
JOB_ASSIGNMENT_MODULE_UPDATED
JOB_ASSIGNMENT_MODULE_DELETED
JOB_ASSIGNMENT_MODULE_ACCESS_DENIED
JOB_ASSIGNMENT_MODULE_EXPORTED
```
