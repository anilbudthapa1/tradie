# 92. Follow-Up Campaign Module — Full Module Specification

## Main Purpose
The **Follow-Up Campaign Module** is responsible for quote/invoice/review follow-up automations in **Tradie Job Manager**.
...
```txt
GET    /api/follow_up_campaign_module
POST   /api/follow_up_campaign_module
GET    /api/follow_up_campaign_module/:id
PATCH  /api/follow_up_campaign_module/:id
DELETE /api/follow_up_campaign_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/follow_up_campaign_module
```
...
```txt
FOLLOW_UP_CAMPAIGN_MODULE_VIEWED
FOLLOW_UP_CAMPAIGN_MODULE_CREATED
FOLLOW_UP_CAMPAIGN_MODULE_UPDATED
FOLLOW_UP_CAMPAIGN_MODULE_DELETED
FOLLOW_UP_CAMPAIGN_MODULE_ACCESS_DENIED
FOLLOW_UP_CAMPAIGN_MODULE_EXPORTED
```
