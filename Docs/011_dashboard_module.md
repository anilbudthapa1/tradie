# 11. Dashboard Module — Full Module Specification

## Main Purpose
The **Dashboard Module** is responsible for owner/employee dashboards, KPIs, alerts and quick actions in **Tradie Job Manager**.
...
```txt
DashboardService, KPIService, AlertService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/dashboard_module
POST   /api/dashboard_module
GET    /api/dashboard_module/:id
PATCH  /api/dashboard_module/:id
DELETE /api/dashboard_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/dashboard_module
```
...
```txt
DASHBOARD_MODULE_VIEWED
DASHBOARD_MODULE_CREATED
DASHBOARD_MODULE_UPDATED
DASHBOARD_MODULE_DELETED
DASHBOARD_MODULE_ACCESS_DENIED
DASHBOARD_MODULE_EXPORTED
```
