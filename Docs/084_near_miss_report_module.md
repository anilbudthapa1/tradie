# 84. Near Miss Report Module — Full Module Specification

## Main Purpose
The **Near Miss Report Module** is responsible for near miss reporting and safety review in **Tradie Job Manager**.
...
```txt
GET    /api/near_miss_report_module
POST   /api/near_miss_report_module
GET    /api/near_miss_report_module/:id
PATCH  /api/near_miss_report_module/:id
DELETE /api/near_miss_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/near_miss_report_module
```
...
```txt
NEAR_MISS_REPORT_MODULE_VIEWED
NEAR_MISS_REPORT_MODULE_CREATED
NEAR_MISS_REPORT_MODULE_UPDATED
NEAR_MISS_REPORT_MODULE_DELETED
NEAR_MISS_REPORT_MODULE_ACCESS_DENIED
NEAR_MISS_REPORT_MODULE_EXPORTED
```
