# 42. Calendar Module — Full Module Specification

## Main Purpose
The **Calendar Module** is responsible for job calendar views for day/week/month in **Tradie Job Manager**.
...
```txt
GET    /api/calendar_module
POST   /api/calendar_module
GET    /api/calendar_module/:id
PATCH  /api/calendar_module/:id
DELETE /api/calendar_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/calendar_module
```
...
```txt
CALENDAR_MODULE_VIEWED
CALENDAR_MODULE_CREATED
CALENDAR_MODULE_UPDATED
CALENDAR_MODULE_DELETED
CALENDAR_MODULE_ACCESS_DENIED
CALENDAR_MODULE_EXPORTED
```
