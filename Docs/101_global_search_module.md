# 101. Global Search Module — Full Module Specification

## Main Purpose
The **Global Search Module** is responsible for search across jobs/customers/invoices/files in **Tradie Job Manager**.
...
```txt
GET    /api/global_search_module
POST   /api/global_search_module
GET    /api/global_search_module/:id
PATCH  /api/global_search_module/:id
DELETE /api/global_search_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/global_search_module
```
...
```txt
GLOBAL_SEARCH_MODULE_VIEWED
GLOBAL_SEARCH_MODULE_CREATED
GLOBAL_SEARCH_MODULE_UPDATED
GLOBAL_SEARCH_MODULE_DELETED
GLOBAL_SEARCH_MODULE_ACCESS_DENIED
GLOBAL_SEARCH_MODULE_EXPORTED
```
