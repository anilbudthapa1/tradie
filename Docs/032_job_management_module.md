# 32. Job Management Module — Full Module Specification

## Main Purpose
The **Job Management Module** is responsible for core job lifecycle and job records in **Tradie Job Manager**.
...
## Core Permissions
```txt
jobs.view_all, jobs.view_assigned, jobs.create, jobs.update
```
...
```txt
GET    /api/job_management_module
POST   /api/job_management_module
GET    /api/job_management_module/:id
PATCH  /api/job_management_module/:id
DELETE /api/job_management_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/job_management_module
```
...
```txt
JOB_MANAGEMENT_MODULE_VIEWED
JOB_MANAGEMENT_MODULE_CREATED
JOB_MANAGEMENT_MODULE_UPDATED
JOB_MANAGEMENT_MODULE_DELETED
JOB_MANAGEMENT_MODULE_ACCESS_DENIED
JOB_MANAGEMENT_MODULE_EXPORTED
```
