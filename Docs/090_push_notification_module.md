# 90. Push Notification Module — Full Module Specification

## Main Purpose
The **Push Notification Module** is responsible for mobile push notifications via FCM/APNs in **Tradie Job Manager**.
...
```txt
GET    /api/push_notification_module
POST   /api/push_notification_module
GET    /api/push_notification_module/:id
PATCH  /api/push_notification_module/:id
DELETE /api/push_notification_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/push_notification_module
```
...
```txt
PUSH_NOTIFICATION_MODULE_VIEWED
PUSH_NOTIFICATION_MODULE_CREATED
PUSH_NOTIFICATION_MODULE_UPDATED
PUSH_NOTIFICATION_MODULE_DELETED
PUSH_NOTIFICATION_MODULE_ACCESS_DENIED
PUSH_NOTIFICATION_MODULE_EXPORTED
```
