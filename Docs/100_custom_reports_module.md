# 100. Custom Reports Module — Full Module Specification

## Main Purpose
The **Custom Reports Module** is responsible for custom report builder later in **Tradie Job Manager**.
...
```txt
GET    /api/custom_reports_module
POST   /api/custom_reports_module
GET    /api/custom_reports_module/:id
PATCH  /api/custom_reports_module/:id
DELETE /api/custom_reports_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/custom_reports_module
```
...
```txt
CUSTOM_REPORTS_MODULE_VIEWED
CUSTOM_REPORTS_MODULE_CREATED
CUSTOM_REPORTS_MODULE_UPDATED
CUSTOM_REPORTS_MODULE_DELETED
CUSTOM_REPORTS_MODULE_ACCESS_DENIED
CUSTOM_REPORTS_MODULE_EXPORTED
```
