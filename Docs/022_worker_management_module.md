# 22. Worker Management Module — Full Module Specification

## Main Purpose
The **Worker Management Module** is responsible for employees/workers profile, access and operational details in **Tradie Job Manager**.
...
```txt
GET    /api/worker_management_module
POST   /api/worker_management_module
GET    /api/worker_management_module/:id
PATCH  /api/worker_management_module/:id
DELETE /api/worker_management_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/worker_management_module
```
...
```txt
WORKER_MANAGEMENT_MODULE_VIEWED
WORKER_MANAGEMENT_MODULE_CREATED
WORKER_MANAGEMENT_MODULE_UPDATED
WORKER_MANAGEMENT_MODULE_DELETED
WORKER_MANAGEMENT_MODULE_ACCESS_DENIED
WORKER_MANAGEMENT_MODULE_EXPORTED
```
