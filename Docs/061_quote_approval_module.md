# 61. Quote Approval Module — Full Module Specification

## Main Purpose
The **Quote Approval Module** is responsible for customer online quote acceptance/rejection in **Tradie Job Manager**.
...
```txt
GET    /api/quote_approval_module
POST   /api/quote_approval_module
GET    /api/quote_approval_module/:id
PATCH  /api/quote_approval_module/:id
DELETE /api/quote_approval_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/quote_approval_module
```
...
```txt
QUOTE_APPROVAL_MODULE_VIEWED
QUOTE_APPROVAL_MODULE_CREATED
QUOTE_APPROVAL_MODULE_UPDATED
QUOTE_APPROVAL_MODULE_DELETED
QUOTE_APPROVAL_MODULE_ACCESS_DENIED
QUOTE_APPROVAL_MODULE_EXPORTED
```
