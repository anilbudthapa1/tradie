# 99. Customer Retention Report Module — Full Module Specification

## Main Purpose
The **Customer Retention Report Module** is responsible for repeat customers and retention analytics in **Tradie Job Manager**.
...
## Core Permissions
```txt
reports.customer_retention.view
```

## Suggested Go Services
```txt
CustomerRetentionReportService
TenantGuard
AuditService
ValidationService
...
```txt
GET    /api/customer_retention_report_module
POST   /api/customer_retention_report_module
GET    /api/customer_retention_report_module/:id
PATCH  /api/customer_retention_report_module/:id
DELETE /api/customer_retention_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/customer_retention_report_module
```
...
```txt
CUSTOMER_RETENTION_REPORT_MODULE_VIEWED
CUSTOMER_RETENTION_REPORT_MODULE_CREATED
CUSTOMER_RETENTION_REPORT_MODULE_UPDATED
CUSTOMER_RETENTION_REPORT_MODULE_DELETED
CUSTOMER_RETENTION_REPORT_MODULE_ACCESS_DENIED
CUSTOMER_RETENTION_REPORT_MODULE_EXPORTED
```
