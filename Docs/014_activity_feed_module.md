# 14. Activity Feed Module — Full Module Specification

## Main Purpose
The **Activity Feed Module** is responsible for tenant activity timeline for important actions in **Tradie Job Manager**.
...
```txt
GET    /api/activity_feed_module
POST   /api/activity_feed_module
GET    /api/activity_feed_module/:id
PATCH  /api/activity_feed_module/:id
DELETE /api/activity_feed_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/activity_feed_module
```
...
```txt
ACTIVITY_FEED_MODULE_VIEWED
ACTIVITY_FEED_MODULE_CREATED
ACTIVITY_FEED_MODULE_UPDATED
ACTIVITY_FEED_MODULE_DELETED
ACTIVITY_FEED_MODULE_ACCESS_DENIED
ACTIVITY_FEED_MODULE_EXPORTED
```
