# 126. Voice Job Notes Module — Full Module Specification

## Main Purpose
The **Voice Job Notes Module** is responsible for voice notes transcription for field workers in **Tradie Job Manager**.
...
```txt
GET    /api/voice_job_notes_module
POST   /api/voice_job_notes_module
GET    /api/voice_job_notes_module/:id
PATCH  /api/voice_job_notes_module/:id
DELETE /api/voice_job_notes_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/voice_job_notes_module
```
...
```txt
VOICE_JOB_NOTES_MODULE_VIEWED
VOICE_JOB_NOTES_MODULE_CREATED
VOICE_JOB_NOTES_MODULE_UPDATED
VOICE_JOB_NOTES_MODULE_DELETED
VOICE_JOB_NOTES_MODULE_ACCESS_DENIED
VOICE_JOB_NOTES_MODULE_EXPORTED
```
