# 122. White Label Module — Full Module Specification

## Main Purpose
The **White Label Module** is responsible for custom branding/domain per business later in **Tradie Job Manager**.
...
```txt
GET    /api/white_label_module
POST   /api/white_label_module
GET    /api/white_label_module/:id
PATCH  /api/white_label_module/:id
DELETE /api/white_label_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/white_label_module
```
...
```txt
WHITE_LABEL_MODULE_VIEWED
WHITE_LABEL_MODULE_CREATED
WHITE_LABEL_MODULE_UPDATED
WHITE_LABEL_MODULE_DELETED
WHITE_LABEL_MODULE_ACCESS_DENIED
WHITE_LABEL_MODULE_EXPORTED
```
