# 95. Revenue Report Module — Full Module Specification

## Main Purpose
The **Revenue Report Module** is responsible for revenue trends, paid/unpaid totals in **Tradie Job Manager**.
...
```txt
GET    /api/revenue_report_module
POST   /api/revenue_report_module
GET    /api/revenue_report_module/:id
PATCH  /api/revenue_report_module/:id
DELETE /api/revenue_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/revenue_report_module
```
...
```txt
REVENUE_REPORT_MODULE_VIEWED
REVENUE_REPORT_MODULE_CREATED
REVENUE_REPORT_MODULE_UPDATED
REVENUE_REPORT_MODULE_DELETED
REVENUE_REPORT_MODULE_ACCESS_DENIED
REVENUE_REPORT_MODULE_EXPORTED
```
