# 54. Quote Management Module — Full Module Specification

## Main Purpose
The **Quote Management Module** is responsible for quote list, statuses and quote lifecycle in **Tradie Job Manager**.
...
```txt
GET    /api/quote_management_module
POST   /api/quote_management_module
GET    /api/quote_management_module/:id
PATCH  /api/quote_management_module/:id
DELETE /api/quote_management_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/quote_management_module
```
...
```txt
QUOTE_MANAGEMENT_MODULE_VIEWED
QUOTE_MANAGEMENT_MODULE_CREATED
QUOTE_MANAGEMENT_MODULE_UPDATED
QUOTE_MANAGEMENT_MODULE_DELETED
QUOTE_MANAGEMENT_MODULE_ACCESS_DENIED
QUOTE_MANAGEMENT_MODULE_EXPORTED
```
