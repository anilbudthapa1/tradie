# 51. ETA Tracking Module — Full Module Specification

## Main Purpose
The **ETA Tracking Module** is responsible for estimate and share arrival times in **Tradie Job Manager**.
...
```txt
GET    /api/eta_tracking_module
POST   /api/eta_tracking_module
GET    /api/eta_tracking_module/:id
PATCH  /api/eta_tracking_module/:id
DELETE /api/eta_tracking_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/eta_tracking_module
```
...
```txt
ETA_TRACKING_MODULE_VIEWED
ETA_TRACKING_MODULE_CREATED
ETA_TRACKING_MODULE_UPDATED
ETA_TRACKING_MODULE_DELETED
ETA_TRACKING_MODULE_ACCESS_DENIED
ETA_TRACKING_MODULE_EXPORTED
```
