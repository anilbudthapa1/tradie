# 96. Jobs Report Module — Full Module Specification

## Main Purpose
The **Jobs Report Module** is responsible for job completion, status and workload reports in **Tradie Job Manager**.
...
```txt
GET    /api/jobs_report_module
POST   /api/jobs_report_module
GET    /api/jobs_report_module/:id
PATCH  /api/jobs_report_module/:id
DELETE /api/jobs_report_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/jobs_report_module
```
...
```txt
JOBS_REPORT_MODULE_VIEWED
JOBS_REPORT_MODULE_CREATED
JOBS_REPORT_MODULE_UPDATED
JOBS_REPORT_MODULE_DELETED
JOBS_REPORT_MODULE_ACCESS_DENIED
JOBS_REPORT_MODULE_EXPORTED
```
