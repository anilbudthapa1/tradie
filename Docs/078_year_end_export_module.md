# 78. Year End Export Module — Full Module Specification

## Main Purpose
The **Year End Export Module** is responsible for year-end financial export package in **Tradie Job Manager**.
...
```txt
GET    /api/year_end_export_module
POST   /api/year_end_export_module
GET    /api/year_end_export_module/:id
PATCH  /api/year_end_export_module/:id
DELETE /api/year_end_export_module/:id
```

For employee/self-service views, use explicit safe endpoints:

```txt
GET /api/me/year_end_export_module
```
...
```txt
YEAR_END_EXPORT_MODULE_VIEWED
YEAR_END_EXPORT_MODULE_CREATED
YEAR_END_EXPORT_MODULE_UPDATED
YEAR_END_EXPORT_MODULE_DELETED
YEAR_END_EXPORT_MODULE_ACCESS_DENIED
YEAR_END_EXPORT_MODULE_EXPORTED
```
